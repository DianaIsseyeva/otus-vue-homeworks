<template>
  <div class="cart-container">
    ...
    <div v-else class="empty-cart" data-testid="empty-cart">
      ...
      <!-- Если используете Vue Router, то <router-link> остаётся -->
      <router-link
        to="/"
...
// Импортируем наш Pinia-хук из "store/index.js"
import { useShopStore } from '@/store'
// или точно укажите путь, например:
// import { useShopStore } from '@/store/index.js'

// Инициализируем store
const shopStore = useShopStore()

// Вычислимые свойства (аналог computed + геттеры в Vuex)
const cart = computed(() => shopStore.cart)
const totalQuantity = computed(() => shopStore.totalQuantity)
const totalPrice = computed(() => shopStore.totalPrice)

// Роутер (для кнопки "Оформить заказ")
const router = useRouter()

// Методы (просто вызываем Pinia actions)
function removeFromCart(productId) {
  shopStore.removeProductFromCart(productId)
}
function clearCart() {
  shopStore.clearCart()
}
