...
<script setup>
import { useShopStore } from '@/store'; // Импорт вашего Pinia-стора
...
// Вместо store.commit('CLEAR_CART') - вызываем метод из Pinia
const clearCart = () => {
  store.clearCart()
}
...
const schema = yup.object().shape({
  ...
  cardHolder: yup
    ...
    .test(
      "cardHolder",
      "Имя на карте должно содержать только буквы и пробелы",
      function (value) {
        const { payment } = this.parent
        if (payment !== "online") return true
        if (!value || value.trim() === "") return false
        return /^[a-zA-Z\s]+$/.test(value)
      }
    )
    .test("cardHolderRequired", "Введите имя владельца карты", function (value) {
      const { payment } = this.parent
      if (payment === "online") {
  ...
  cardNumber: yup
    .string()
    .test("cardNumber", "Номер карты должен содержать 16 цифр", function (value) {
      const { payment } = this.parent
      if (payment !== "online") return true
      if (!value || value.trim() === "") return false
      return /^\d{16}$/.test(value)
    })
    .test("cardNumberRequired", "Введите номер карты", function (value) {
      const { payment } = this.parent
      if (payment === "online") {
        return value && value.trim() !== ""
      }
  ...
  expiryDate: yup
    .string()
    .test("expiryDateFormat", "Введите дату в формате MM/YY", function (value) {
      const { payment } = this.parent
      if (payment !== "online") return true
      if (!value || value.trim() === "") return false
      return /^(0[1-9]|1[0-2])\/?([0-9]{2})$/.test(value)
    })
    .test("expiryDateValid", "Карта просрочена", function (value) {
      const { payment } = this.parent
      if (payment !== "online") return true
    ...
    .test("expiryDateRequired", "Введите дату окончания", function (value) {
      const { payment } = this.parent
      if (payment === "online") {
        return value && value.trim() !== ""
      }
  ...
  cvc: yup
    .string()
    .test("cvcFormat", "CVC должен содержать ровно 3 цифры", function (value) {
      const { payment } = this.parent
      if (payment !== "online") return true
      if (!value || value.trim() === "") return false
      return /^\d{3}$/.test(value)
    })
    .test("cvcRequired", "Введите CVC", function (value) {
      const { payment } = this.parent
      if (payment === "online") {
        return value && value.trim() !== ""
      }
  ...
  try {
    ...
    if (validationErrors.inner) {
      validationErrors.inner.forEach((error) => {
        // Ошибки во вложенных полях (address.city и т.п.)
...
// Изменение формы при смене метода оплаты
